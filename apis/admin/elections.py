...
from admins.models import Election ,User
...
async def all_elections(session:Session =Depends(get_connection),current_admin :User =Depends(get_current_admin)):
...
async def search_a_election(election_id:int ,session:Session =Depends(get_connection), current_admin :User =Depends(get_current_admin)):
...
async def delete_a_election(election_id:int ,session:Session =Depends(get_connection),current_admin :User =Depends(get_current_admin)):
...
async def adding_a_election(new_election : CreateElection,session:Session = Depends(get_connection),current_admin :User =Depends(get_current_admin)):
...
async def updating_a_election(election_id:int ,updated_data:UpdateElection,session:Session=Depends(get_connection), current_admin :User =Depends(get_current_admin)):
